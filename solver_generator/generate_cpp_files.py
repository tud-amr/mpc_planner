...
def generate_cpp_code(settings, model):
    ...
    for key, indices in settings["params"].parameter_bundles.items():
        ...
        if len(indices) == 1:
            header_file.write(f"void setForcesParameter{function_name}(int k, Solver_params& params, const double value, int index=0);\n")
            cpp_file.write(f"void setForcesParameter{function_name}(int k, Solver_params& params, const double value, int index){{\n")
            cpp_file.write("\t(void)index;\n")
            cpp_file.write(f"\tparams.all_parameters[k * {settings['params'].length()} + {indices[0]}] = value;\n")
        else:
            header_file.write(f"void setForcesParameter{function_name}(int k, Solver_params& params, const double value, int index);\n")
            cpp_file.write(f"void setForcesParameter{function_name}(int k, Solver_params& params, const double value, int index){{\n")
