...
class SplineXY:
    ...
    def compute_point(self, spline_index):
        ...
        self.path_dx = (
            3 * self.x_a * (spline_index - self.s_start) ** 2
            + 2 * self.x_b * (spline_index - self.s_start)
            + self.x_c
        )

        self.path_dy = (
            3 * self.y_a * (spline_index - self.s_start) ** 2
...
class ContouringObjective:
    ...
    def get_value(self, model, params, settings, stage_idx):
        ...
        for i in range(self.num_segments):
            ...
            if i > 0:
                lambdas.append(
                    1.0 / (1.0 + np.exp((s - splines[-1].s_start + 0.02) / 0.1))
                )  # Sigmoid
        ...
        for k in range(len(splines) - 1, 0, -1):
            ...
            path_x = (
                lambdas[k - 1] * splines[k - 1].path_x + (1.0 - lambdas[k - 1]) * path_x
            )
            path_y = (
                lambdas[k - 1] * splines[k - 1].path_y + (1.0 - lambdas[k - 1]) * path_y
            )
            path_dx = (
                lambdas[k - 1] * splines[k - 1].path_dx
                + (1.0 - lambdas[k - 1]) * path_dx
            )
            path_dy = (
                lambdas[k - 1] * splines[k - 1].path_dy
                + (1.0 - lambdas[k - 1]) * path_dy
            )
        ...
        contour_error = path_dy_normalized * (pos_x - path_x) - path_dx_normalized * (
            pos_y - path_y
        )
        lag_error = -path_dx_normalized * (pos_x - path_x) - path_dy_normalized * (
            pos_y - path_y
        )
...
class ContouringModule(ObjectiveModule):
    ...
    def __init__(self, settings, num_segments):
        super().__init__()
        self.module_name = (
            "Contouring"  # Needs to correspond to the c++ name of the module
        )
