...
def declare_forces(header_file, cpp_file, name, solver_name):
    ...
    cpp_file.write("extern \"C\"\n{\n"
                   "\t"+solver_name+"_extfunc extfunc_eval_"+name.lower()+" = &"+solver_name+"_adtool2forces;\n"
                    "}\n")
...
def add_reset_solver(header_file, cpp_file):
    ...
    cpp_file.write("\t\tfor (long int i = 0; i < *(&_params.all_parameters + 1) - _params.all_parameters; i++)\n"
...
def add_state_and_inputs(header_file, cpp_file, model):
    ...
    for i in range(0, len(model.inputs)):
        header_file.write("\tdouble& " + model.inputs[i] + \
                          "(unsigned int k = 0) { return _params.x0[k * nvar + " + str(i) + "]; };\n")
...
def add_set_and_get_parameter(header_file, cpp_file):
    ...
    open_function(header_file, cpp_file, "void setParameter(int k, std::string&& parameter, double value)")
    cpp_file.write("\t\t_params.all_parameters[k*npar + _parameter_map[parameter].as<int>()] = value;\n")
    close_function(cpp_file)
    
    open_function(header_file, cpp_file, "double getParameter(int k, std::string&& parameter)")
...
def add_solve(header_file, cpp_file, name, forces_solver_name):
    ...
    cpp_file.write("\t\tint exit_code = "+forces_solver_name+"_solve(&_params, &_output, &_info, _solver_memory_handle, stdout, extfunc_eval_"+name.lower()+");\n")
...
def add_get_output(header_file, cpp_file, settings):
    open_function(header_file, cpp_file, "double getOutput(int k, std::string&& state_name)")
...
def generate_cpp_code_old(settings, model):    
    ...
    header_file.write(\
        "\t"+forces_solver_name+"_params _params;\n" +
        "\t"+forces_solver_name+"_output _output;\n" +
        "\t"+forces_solver_name+"_info _info;\n\n")
    ...
    open_function(header_file, cpp_file, "Solver(int solver_id)",
                  optional_header_with_defaults="Solver(int solver_id = 0)",
                  has_type=False)
    cpp_file.write("\t_solver_id = solver_id;\n"
                   "\t\t_solver_memory = (char*)malloc(" + forces_solver_name + "_get_mem_size());\n"
		            "\t\t_solver_memory_handle = " + forces_solver_name + "_external_mem(_solver_memory, _solver_id," + forces_solver_name+ "_get_mem_size());\n"
    ...
    add_state_and_inputs(header_file, cpp_file, model) # TODO: Is it clear here that this refers to x0?
    ...
    if config.use_sqp and (not config.use_scenario_constraints):
        ...
        cpp_file.write("\t\tint exit_code = -1;\n"
                          ...
                          "\t\t\texit_code = " + model.system.name + "FORCESNLPsolver_solve(&forces_params_, &forces_output_, "
                                                              "&forces_info_, solver_memory_handle_, stdout, extfunc_eval_" + model.system.name.lower()+");\n"
    ...
    if config.use_sqp:
        if config.sqp_save_bfgs:
            cpp_file.write("\t\tstd::cout << \"Saving BFGS \\n\";\n"
    ...
    cpp_file.write("\t\treturn was_safe_solution_; // True if the last solution has converged\n")
    close_function(cpp_file)
    ...
    for idx, state in enumerate(model.states):
        ...
        cpp_file.write(
                "\t\tplan_.states_[" + str(config.N-1) + "].set_" + state + "(forces_output_.x" + val_zero(config.N + 1) + "[" + str(
                    idx + model.nu) + "]);\n")
    ...
    if config.use_sqp and config.sqp_supply_bfgs:
        ...
        for i in range(model.nx):
            ...
            cpp_file.write("if(i == " + str(idx) + ") {value = " + str(model.bfgs_init[i, i]) + ";}\n")
            idx += didx
            didx += 1
    ...
    if config.use_sqp and config.sqp_supply_bfgs:
        ...
        for k in range(settings.N_bar):
            if k < 10:
                cpp_file.write("\t\t\tforces_params_.BFGSinitLower0" + str(k) + "[i] = forces_output_.")
            else:
                cpp_file.write("\t\t\tforces_params_.BFGSinitLower" + str(k) + "[i] = forces_output_.")
    ...
    cpp_file.write("\t\t/* Pre-determined size and precision settings */\n"
                      ...
                      "]), both with length N = " + str(config.N) + "\" << std::endl;\n"
